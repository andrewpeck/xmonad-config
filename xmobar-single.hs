...
Config {
    ...
    persistent = True,
    commands = [
        -- https://www.faa.gov/air_traffic/weather/asos/?state=MA
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %multicpu%   %memory%   %swap%  %enp4s0%      <fc=#FFFFCC>%date%</fc> %KBOS%"
    --template = "%StdinReader% }{ %multicpu%   %memory%   %swap%  %eth0%   Vol: <fc=#b2b2ff>%default:Master%</fc>   <fc=#FFFFCC>%date%</fc>"
}
